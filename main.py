...
data['profit_cpt'] = 100.0 * data['profit'] / data['sales']
...
with mx_profit_pct:
    curr_profit_cpt = data.loc[data['order_year'] == CURR_YEAR, 'profit_cpt'].values[0]
    prev_profit_cpt = data.loc[data['order_year'] == PREV_YEAR, 'profit_cpt'].values[0]

    profit_cpt_diff_pct = 100.0 * (curr_profit_cpt - prev_profit_cpt) / prev_profit_cpt

    st.metric("Profit", value=f'{curr_profit_cpt:.2f}%', delta=f'{profit_cpt_diff_pct:.2f}%')
